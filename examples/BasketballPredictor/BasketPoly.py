...
for video_number in range(1, 8):
    print(video_number)
    cap = cv2.VideoCapture(f'examples/BasketballPredictor/test_images/Videos/vid ({video_number}).mp4')
    ...
    while cap.isOpened():
        ...
        if not success:
            break
        # img = cv2.imread("examples/BasketballPredictor/test_images/Ball.png")
        img = img[0:900,:]
